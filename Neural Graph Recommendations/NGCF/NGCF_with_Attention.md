...
We consider a bipartite graph \( G_r = (U, I, E) \), where \( U = \{u_1, \dots, u_N\} \), \( I = \{i_1, \dots, i_M\} \), and edges \( E \subseteq U \times I \) represent user-item interactions. Each node has an embedding vector \( \mathbf{e}_u, \mathbf{e}_i \in \mathbb{R}^d \), initialized randomly or using side attributes.
